"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || function (mod) {
    if (mod && mod.__esModule) return mod;
    var result = {};
    if (mod != null) for (var k in mod) if (k !== "default" && Object.prototype.hasOwnProperty.call(mod, k)) __createBinding(result, mod, k);
    __setModuleDefault(result, mod);
    return result;
};
Object.defineProperty(exports, "__esModule", { value: true });
const express_1 = require("express");
const middlewares_1 = require("../../middlewares");
const TodoHandler = __importStar(require("./todo.handler"));
const userTodo_1 = require("../../models/userTodo");
const zod_1 = require("zod");
const router = (0, express_1.Router)();
router.get('/', middlewares_1.validateAuthentication, (0, middlewares_1.validateRequest)({
    query: zod_1.z.object({
        week: zod_1.z.string().optional()
    }),
}), TodoHandler.getAll);
router.post('/', middlewares_1.validateAuthentication, (0, middlewares_1.validateRequest)({
    body: userTodo_1.UserTodo.omit({
        completed: true,
        adminTodo: true,
        completedOn: true,
        userId: true,
        priority: true,
        week: true,
    }).extend({
        completionDate: zod_1.z.string().datetime().optional(),
    }),
}), TodoHandler.createOne);
router.patch('/:id', middlewares_1.validateAuthentication, (0, middlewares_1.validateRequest)({
    body: userTodo_1.UserTodo.omit({
        completed: true,
        completedOn: true,
        priority: true,
        week: true,
        adminTodo: true,
        userId: true,
    }).extend({
        admin: zod_1.z.boolean(),
        title: zod_1.z.string().optional(),
        description: zod_1.z.string().optional(),
        userPriority: zod_1.z.enum(['normal', 'high', 'crucial']).optional(),
        completionDate: zod_1.z.string().datetime().optional(),
        assignPartner: zod_1.z.boolean().optional(),
    }),
}), TodoHandler.updateOne);
router.post('/:id/complete', middlewares_1.validateAuthentication, (0, middlewares_1.validateRequest)({
    params: zod_1.z.object({
        id: zod_1.z.string()
    }),
    body: zod_1.z.object({
        admin: zod_1.z.boolean().default(false),
    }),
}), TodoHandler.completeOne);
router.post('/:id/incomplete', middlewares_1.validateAuthentication, (0, middlewares_1.validateRequest)({
    params: zod_1.z.object({
        id: zod_1.z.string()
    }),
    body: zod_1.z.object({
        admin: zod_1.z.boolean().default(false),
    }),
}), TodoHandler.incompleteOne);
router.delete('/:id', middlewares_1.validateAuthentication, (0, middlewares_1.validateRequest)({
    params: zod_1.z.object({
        id: zod_1.z.string()
    }),
}), TodoHandler.deleteOne);
exports.default = router;
//# sourceMappingURL=todo.route.js.map