...
const getWeekFromUser = async (user, reqWeek, calander) => {
    try {
        ...
        let days = [];
        if (user?.partnerId) {
            const partneredUser = await user_1.Users.findOne({ _id: user.partnerId });
            if (partneredUser) {
                week = (0, exports.getCurrentWeek)(partneredUser.stage, partneredUser.createdAt);
            ...
            if (partneredUser?.consiveDate) {
                const cw = (0, exports.getCurrentWeekFromConsiveDate)(partneredUser.consiveDate, partneredUser.createdAt);
                week = cw.week;
                date = cw.date;
                week = reqWeek ? reqWeek : week;
                if (calander) {
